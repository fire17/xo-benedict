...
def print_sentence_with_color(sentence="Hello, world!", color=None):
    console = Console()

    for named_color in named_colors:
        style = f"{'' if color else 'bold '}{named_color}" if color is None else f"{'' if color.startswith('bold') else 'bold '}{color}"
        console.print(sentence, style=style)

print_sentence_with_color()
...
def print_sentence_with_color(sentence="Hello, world!", color=None):
    ...
    # Split the named colors into groups of 4
    split_colors = [named_colors[i:i+4] for i in range(0, len(named_colors), 4)]

    for group in split_colors:
        table = Table(show_header=False, title="Colors")
        table.add_column("Color Name")
        table.add_column("Example Sentence")

        for named_color in group:
            style = f"{'' if color else 'bold '}{named_color}" if color is None else f"{'' if color.startswith('bold') else 'bold '}{color}"
            table.add_row(named_color, f"[{style}]{sentence}[/{style}]")
...
def print_sentence_with_color(sentence="Hello, world!", color=None):
    ...
    for i in range(len(named_colors)//cols):
        # Apply style if color is specified
        row = []
        for j in range(cols):
            style = f"{'' if color else 'bold '}{named_colors[i+j*len(named_colors)//cols]}" if color is None else f"{'' if color.startswith('bold') else 'bold '}{color}"
            # Add color name and example sentence to the table
            row.append(named_colors[i+j*len(named_colors)//cols])
            row.append(f"[{style}]{sentence}[/{style}]")
        # print("ROW",row)
        table.add_row(*row)
